...
class TupleList:
    ...
    def convert_tuple_list_to_microseconds(self, distrib_mod):
        microseconds_tuple_list = [] # Each index in this tuple list represents 1 millisecond of packets to process
        minute_counter = 0
        #second_counter = 0
        for tuple in self.tuple_list:
            load_size = float(int(tuple[0])/1000000)
            for s_iterator in range(60):
                for ms_iterator in range(1000000):
                    ...
                    tempTuple = (randomized_load_size, minute_counter*60000000+s_iterator*1000000+ms_iterator)
                    microseconds_tuple_list.append(tempTuple)
            minute_counter += 1
        return microseconds_tuple_list
